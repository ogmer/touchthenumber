// 効果音・BGMのWAVファイルを生成するスクリプト
// 実行: dart run tool/generate_sounds.dart
// 生成後、BGMは `py tool/compress_bgm.py` でMP3に圧縮してアプリサイズを削減する
// （効果音は短いのでWAVのまま。BGMは長く非圧縮だと数MBになるため）
import 'dart:io';
import 'dart:math';
...
/// ヨナ抜き音階のメロディ・小節頭のベルを重ねた12小節（約27秒）。
/// C・F・G・Am だけの素直なダイアトニック進行（転調や意外なコードは入れない）で、
/// 最後のCへ気持ちよく解決してループする
List<double> _titleBgmTrack() {
  ...
  // コード進行: C F G C / F G Am C / F G C C（12小節、素直なダイアトニックのみ）。
  // Cで解決してループ頭のCへ自然につながる
  const chords = [
    [60, 64, 67], // C:  C4 E4 G4
  ...
  const bassRoots = [48, 53, 55, 48, 53, 55, 45, 48, 53, 55, 48, 48];

  // メロディ: [8分音符位置, MIDIノート, 長さ(8分音符数)]。
  // ヨナ抜き音階（ド・レ・ミ・ソ・ラ）の高めの音域で、動きのある明るいフレーズを
  // 12小節分。大きな跳躍や不自然な音は避け、素直に歌わせる
  const melody = [
    ...
    [[0, 72, 2], [2, 74, 1], [3, 76, 1], [4, 74, 2], [6, 72, 2]],
    [[0, 72, 2], [2, 76, 1], [3, 79, 1], [4, 84, 2], [6, 79, 2]],
    [[0, 81, 2], [2, 79, 1], [3, 76, 1], [4, 79, 2], [6, 81, 2]],
    [[0, 79, 2], [2, 76, 1], [3, 74, 1], [4, 76, 2], [6, 79, 2]],
    [[0, 81, 2], [2, 79, 1], [3, 76, 1], [4, 74, 2], [6, 72, 2]],
    [[0, 74, 2], [2, 76, 1], [3, 79, 1], [4, 76, 2], [6, 74, 2]],
  ];
